...
const TableComponentView: React.FC<TableComponentProps> = ({
  entityName,
  extraParams = {},
  title,
}) => {
  ...
  const renderOrderContent = () => {
    return usedData.map((row: any, idx) => {
      const orderDate = new Date(row.createdAt);
      const totalItems =
        row.items?.reduce((acc: number, item: any) => acc + item.quantity, 0) ||
        0;

      return (
        <tr key={idx + 1}>
          <td>
            <Typography level="body-xs">{row.orderNumber}</Typography>
          </td>
          <td>
            <Typography level="body-xs">
          ...
          <td>
            <Typography level="body-xs">{totalItems} items</Typography>
          </td>
          <td>
            <Typography level="body-xs">
  ...
  const renderContent = () => {
    if (usedData.length > 0) {
      if (entityName === "products") {
        ...
        return renderOrderContent(); // Tambahkan fungsi ini
      } else {
  ...
  const getTableHeaders = () => {
    if (entityName === "products") {
      ...
      return (
        <thead>
          <tr>
            <th style={{width: 120, padding: "0.5rem"}}>Order Number</th>
            <th style={{width: 120, padding: "0.5rem"}}>Tanggal</th>
            <th style={{width: 150, padding: "0.5rem"}}>Customer</th>
            <th style={{width: 80, padding: "0.5rem"}}>Items</th>
            <th style={{width: 100, padding: "0.5rem"}}>Total</th>
            <th style={{width: 100, padding: "0.5rem"}}>Status Order</th>
            <th style={{width: 100, padding: "0.5rem"}}>Status Payment</th>
            <th style={{width: 80, padding: "0.5rem"}}> </th>
          </tr>
        </thead>
      );
  ...
  const getColSpan = () => {
    if (entityName === "products") return 7;
    if (entityName === "orders") return 8; // 8 kolom untuk orders
    return 5;
  };
